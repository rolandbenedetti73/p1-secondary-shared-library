...
const TONES: Record<TestimonialProps["tone"], { wrap: string; onDark: boolean; accent: string }> = {
  ...
  white: { wrap: "bg-p1-bg-default text-p1-text", onDark: false, accent: "text-p1-primary" },
...
export const TestimonialBlock: ComponentConfig<TestimonialProps> = {
  ...
  defaultProps: {
    quote:
      "The team was up and running in a day, and we haven’t looked back. It just works.",
    name: "Jordan Ellis",
  ...
  render: ({ quote, name, role, avatarUrl, layout, tone }) => {
    const t = TONES[tone];
    ...
    return (
      <div className={`px-p1-lg py-p1-xl ${t.wrap}`}>
        <div
          className={`mx-auto flex flex-col gap-p1-lg ${large ? "max-w-4xl" : "max-w-3xl"} ${
            center ? "text-center" : "text-left"
          }`}
        >
          <div className={`font-serif text-5xl leading-none ${t.accent}`}>“</div>
          <p
            className={`font-serif italic font-medium leading-snug text-balance ${
          ...
          <div className={`flex items-center gap-p1-sm ${center ? "justify-center" : "justify-start"}`}>
            ...
            <div className="text-left">
              ...
              <div className={`text-sm ${t.onDark ? "text-white/70" : "text-p1-text-muted"}`}>
                {role}
              </div>
            </div>
          </div>
